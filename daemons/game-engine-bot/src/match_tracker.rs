...
use crate::match_state_machine::{MatchState, MatchEvent, TransitionResult, GameEngineAction};
...
impl MatchTracker {
    ...
    pub async fn process_event(&self, event: PlayerMatchEvent) -> Result<(), GameEngineError> {
        ...
        if matches.len() >= self.max_concurrent_matches && !matches.contains_key(&match_id) {
            warn!("🚫 Maximum concurrent matches ({}) reached", self.max_concurrent_matches);
            return Err(GameEngineError::Internal("Too many concurrent matches".to_string()));
        }
        
        let current_state = matches.get(&match_id)
            ...
            .unwrap_or_else(|| {
                ...
                match &match_event {
                    ...
                    _ => {
                        warn!("🚨 Received non-challenge event for unknown match: {}", match_id);
        ...
        let tracked_match = TrackedMatch {
            ...
            created_at: matches.get(&match_id)
                .map(|tm| tm.created_at)
                .unwrap_or_else(|| Utc::now()),
            last_updated: Utc::now(),
            action_count: matches.get(&match_id)
        ...
        info!("🎮 Match {} transitioned to: {}", match_id, transition_result.new_state.phase_name());
    ...
    async fn convert_to_match_event(&self, event: PlayerMatchEvent) -> Result<(String, MatchEvent), GameEngineError> {
        match event {
    ...
    pub async fn cleanup_expired_matches(&self) {
        ...
        for match_id in expired_matches {
            if let Some(tracked_match) = matches.remove(&match_id) {
                warn!("⏰ Expired match removed: {} (last updated: {})", 
    ...
    pub async fn invalidate_match(&self, match_id: &str, reason: String) -> Result<(), GameEngineError> {
        let mut matches = self.matches.write().await;
        
        if let Some(tracked_match) = matches.get_mut(match_id) {
            let transition_result = tracked_match.state.clone().transition(
                MatchEvent::InvalidationTriggered(reason.clone())
            );
...
pub async fn run_cleanup_task(tracker: Arc<MatchTracker>) {
    ...
    loop {
        ...
        debug!("🧹 Cleanup cycle: {} total matches, {} active", 
               stats.total_matches, stats.active_matches());
    }
}