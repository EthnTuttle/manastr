...
impl PlayerMatch {
    ...
    pub fn add_token_reveal(&mut self, reveal: &TokenReveal) -> Result<(), String> {
        if !matches!(self.phase, MatchPhase::Accepted | MatchPhase::TokensRevealed) {
    ...
    pub fn add_move_commitment(&mut self, commitment: &MoveCommitment) -> Result<(), String> {
        ...
        if commitment.player_npub == self.player1_npub {
            self.player1_commitments.moves_by_round.insert(round, commitment.move_commitment.clone());
        } else if commitment.player_npub == self.player2_npub {
            self.player2_commitments.moves_by_round.insert(round, commitment.move_commitment.clone());
        } else {
            return Err("Move commitment from unknown player".to_string());
        }

        debug!("Added move commitment for round {} from {}", round, commitment.player_npub);
        Ok(())
    }
    ...
    pub fn add_move_reveal(&mut self, reveal: &MoveReveal) -> Result<(), String> {
        ...
        if reveal.player_npub == self.player1_npub {
            self.player1_reveals.moves_by_round.insert(
                round, 
                (reveal.unit_positions.clone(), reveal.unit_abilities.clone(), reveal.moves_nonce.clone())
            );
        } else if reveal.player_npub == self.player2_npub {
        ...
        debug!("Added move reveal for round {} from {}", round, reveal.player_npub);
        Ok(())
    }
...
impl MatchChallenge {
    pub fn to_nostr_event(&self, keys: &Keys) -> Result<Event, Box<dyn std::error::Error>> {
        ...
        let tags = vec![
            Tag::custom(nostr::TagKind::Custom("d".into()), vec![self.challenger_npub.clone()]),
            Tag::custom(nostr::TagKind::Custom("wager".into()), vec![self.wager_amount.to_string()]),
            Tag::custom(nostr::TagKind::Custom("league".into()), vec![self.league_id.to_string()]),
            Tag::custom(nostr::TagKind::Custom("expires".into()), vec![self.expires_at.to_string()]),
        ];
...
impl MatchAcceptance {
    pub fn to_nostr_event(&self, keys: &Keys, challenge_event_id: &str) -> Result<Event, Box<dyn std::error::Error>> {
        let content = serde_json::to_string(self)?;
        let tags = vec![
            Tag::event(nostr::EventId::from_hex(challenge_event_id)?),
            Tag::custom(nostr::TagKind::Custom("wager".into()), vec!["100".to_string()]), // TODO: Use actual wager
        ];
...
impl TokenReveal {
    pub fn to_nostr_event(&self, keys: &Keys, match_event_id: &str) -> Result<Event, Box<dyn std::error::Error>> {
        ...
        let tags = vec![
            ...
            Tag::custom(nostr::TagKind::Custom("phase".into()), vec!["token_reveal".to_string()]),
        ];
...
impl MoveCommitment {
    pub fn to_nostr_event(&self, keys: &Keys, match_event_id: &str) -> Result<Event, Box<dyn std::error::Error>> {
        ...
        let tags = vec![
            Tag::event(nostr::EventId::from_hex(match_event_id)?),
            Tag::custom(nostr::TagKind::Custom("round".into()), vec![self.round_number.to_string()]),
            Tag::custom(nostr::TagKind::Custom("phase".into()), vec!["move_commit".to_string()]),
        ];
...
impl MoveReveal {
    pub fn to_nostr_event(&self, keys: &Keys, match_event_id: &str) -> Result<Event, Box<dyn std::error::Error>> {
        ...
        let tags = vec![
            Tag::event(nostr::EventId::from_hex(match_event_id)?),
            Tag::custom(nostr::TagKind::Custom("round".into()), vec![self.round_number.to_string()]),
            Tag::custom(nostr::TagKind::Custom("phase".into()), vec!["move_reveal".to_string()]),
        ];
...
impl MatchResult {
    pub fn to_nostr_event(&self, keys: &Keys, match_event_id: &str) -> Result<Event, Box<dyn std::error::Error>> {
        let content = serde_json::to_string(self)?;
        let winner_tag = self.calculated_winner.as_ref().unwrap_or(&"draw".to_string()).clone();
        
        let tags = vec![
            Tag::event(nostr::EventId::from_hex(match_event_id)?),
            Tag::custom(nostr::TagKind::Custom("winner".into()), vec![winner_tag]),
            Tag::custom(nostr::TagKind::Custom("phase".into()), vec!["match_complete".to_string()]),
        ];
