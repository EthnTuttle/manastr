...
impl MatchValidationManager {
    ...
    pub fn initialize_match_validation(&mut self, acceptance: &MatchAcceptance) -> Result<(), GameEngineError> {
        ...
        self.matches.insert(acceptance.match_event_id.clone(), player_match);
    ...
    pub fn get_match(&self, match_event_id: &str) -> Result<&PlayerMatch, GameEngineError> {
        self.matches.get(match_event_id)
            .ok_or_else(|| GameEngineError::MatchNotFound(match_event_id.to_string()))
    }

    /// Get mutable PlayerMatch for validation updates
    pub fn get_match_mut(&mut self, match_event_id: &str) -> Result<&mut PlayerMatch, GameEngineError> {
        self.matches.get_mut(match_event_id)
            .ok_or_else(|| GameEngineError::MatchNotFound(match_event_id.to_string()))
    }
    ...
    pub fn is_ready_for_final_validation(&self, match_event_id: &str) -> Result<bool, GameEngineError> {
    ...
    pub fn validate_match_result(&self, match_event_id: &str, claimed_result: &crate::match_events::MatchResult) -> Result<ValidationSummary, GameEngineError> {
        use tracing::{debug, info, warn, error};
        use shared_game_logic::combat::{generate_units_from_token_secret, process_combat};
        
        info!("🔍 Starting comprehensive match validation for {}", match_event_id);
        ...
        let (player1_army, player2_army) = match self.generate_validated_armies(player_match) {
            Ok(armies) => {
                ...
                debug!("  Player 1 ({}): {} units", player_match.player1_npub, armies.0.len());
                debug!("  Player 2 ({}): {} units", player_match.player2_npub, armies.1.len());
                armies
            },
            Err(e) => {
        ...
        let validated_rounds = match self.validate_all_combat_rounds(
            ...
            Ok(rounds) => {
                info!("✅ All {} combat rounds validated successfully", rounds.len());
                rounds
            },
            Err(e) => {
        ...
        if calculated_winner == claimed_result.calculated_winner {
            ...
            warn!("❌ Winner mismatch - Expected: {:?}, Claimed: {:?}", 
        ...
        if validation.commitments_valid && validation.combat_verified && validation.winner_confirmed {
            info!("🎉 MATCH VALIDATION COMPLETE: All checks passed for {}", match_event_id);
        } else {
            warn!("⚠️ MATCH VALIDATION FAILED: Some checks failed for {}", match_event_id);
        }
    ...
    fn validate_all_commitments(&self, player_match: &PlayerMatch) -> Result<(), GameEngineError> {
        ...
        let p1_nonce = player_match.player1_reveals.token_nonce.as_ref()
            .ok_or_else(|| GameEngineError::Internal("Player 1 missing token nonce".to_string()))?;
        let p1_commitment = player_match.player1_commitments.cashu_tokens.as_ref()
            .ok_or_else(|| GameEngineError::Internal("Player 1 missing token commitment".to_string()))?;
        ...
        let p2_nonce = player_match.player2_reveals.token_nonce.as_ref()
            .ok_or_else(|| GameEngineError::Internal("Player 2 missing token nonce".to_string()))?;
        let p2_commitment = player_match.player2_commitments.cashu_tokens.as_ref()
            .ok_or_else(|| GameEngineError::Internal("Player 2 missing token commitment".to_string()))?;
        ...
        let completed_rounds: Vec<u32> = player_match.player1_reveals.moves_by_round
            .keys()
            .filter(|&round| player_match.player2_reveals.moves_by_round.contains_key(round))
            .cloned()
            .collect();
        
        info!("🎯 Validating move commitments for {} completed rounds", completed_rounds.len());
        
        for round in completed_rounds {
            debug!("🔍 Validating round {} move commitments", round);
            
            // Player 1 move validation
            let p1_move_data = player_match.player1_reveals.moves_by_round.get(&round)
                .ok_or_else(|| GameEngineError::Internal(format!("Player 1 missing moves for round {}", round)))?;
            let p1_move_commitment = player_match.player1_commitments.moves_by_round.get(&round)
                .ok_or_else(|| GameEngineError::Internal(format!("Player 1 missing move commitment for round {}", round)))?;
            ...
            if !verify_moves_commitment(
                ...
                return Err(GameEngineError::Internal(
                    format!("Player 1 move commitment verification failed for round {}", round)
                ));
            }
            
            // Player 2 move validation
            let p2_move_data = player_match.player2_reveals.moves_by_round.get(&round)
                .ok_or_else(|| GameEngineError::Internal(format!("Player 2 missing moves for round {}", round)))?;
            let p2_move_commitment = player_match.player2_commitments.moves_by_round.get(&round)
                .ok_or_else(|| GameEngineError::Internal(format!("Player 2 missing move commitment for round {}", round)))?;
            ...
            if !verify_moves_commitment(
                ...
                return Err(GameEngineError::Internal(
                    format!("Player 2 move commitment verification failed for round {}", round)
                ));
            }
    ...
    fn generate_validated_armies(&self, player_match: &PlayerMatch) -> Result<([shared_game_logic::game_state::Unit; 8], [shared_game_logic::game_state::Unit; 8]), GameEngineError> {
        use tracing::{debug, info};
        use shared_game_logic::combat::generate_units_from_token_secret;
        
        // Get token secrets (first token used for army generation)
        let p1_tokens = player_match.player1_reveals.cashu_tokens.as_ref()
            .ok_or_else(|| GameEngineError::Internal("Player 1 tokens not revealed".to_string()))?;
        let p2_tokens = player_match.player2_reveals.cashu_tokens.as_ref()
            .ok_or_else(|| GameEngineError::Internal("Player 2 tokens not revealed".to_string()))?;
        
        if p1_tokens.is_empty() || p2_tokens.is_empty() {
            return Err(GameEngineError::Internal("Players must have at least one token".to_string()));
        }
        
        info!("🏭 Generating armies using deterministic algorithm:");
        info!("  Player 1 ({}): Using token secret from {} tokens", player_match.player1_npub, p1_tokens.len());
        info!("  Player 2 ({}): Using token secret from {} tokens", player_match.player2_npub, p2_tokens.len());
        ...
        info!("✅ Both armies generated successfully using league {} modifiers", player_match.league_id);
    ...
    fn validate_all_combat_rounds(
        ...
        info!("⚔️ Re-executing {} combat rounds for validation", completed_rounds.len());
        ...
        for round_num in completed_rounds {
            ...
            let p1_moves = player_match.player1_reveals.moves_by_round.get(&round_num)
                .ok_or_else(|| GameEngineError::Internal(format!("Player 1 moves missing for round {}", round_num)))?;
            let p2_moves = player_match.player2_reveals.moves_by_round.get(&round_num)
                .ok_or_else(|| GameEngineError::Internal(format!("Player 2 moves missing for round {}", round_num)))?;
            
            // Extract unit positions (which units to use)
            let p1_unit_idx = p1_moves.0.get(0).copied().unwrap_or(0) as usize % 8;
            let p2_unit_idx = p2_moves.0.get(0).copied().unwrap_or(0) as usize % 8;
            
            debug!("🎯 Round {} unit selection:", round_num);
            debug!("  Player 1 selected unit {} (from position bytes: {:?})", p1_unit_idx, p1_moves.0);
            debug!("  Player 2 selected unit {} (from position bytes: {:?})", p2_unit_idx, p2_moves.0);
            ...
            debug!("  P1 Unit: ATK={} DEF={} HP={} ABILITY={:?}", 
                   p1_unit.attack, p1_unit.defense, p1_unit.health, p1_unit.ability);
            debug!("  P2 Unit: ATK={} DEF={} HP={} ABILITY={:?}", 
        ...
        info!("✅ All {} rounds validated successfully", validated_rounds.len());
        Ok(validated_rounds)
    }
    ...
    fn calculate_match_winner(
        ...
        info!("🏆 Calculating match winner from {} rounds", validated_rounds.len());
        ...
        info!("📊 Final score - Player 1: {} wins, Player 2: {} wins", p1_wins, p2_wins);
