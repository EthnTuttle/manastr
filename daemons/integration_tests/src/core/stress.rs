...
pub async fn test_stress_scenarios(core: &TestSuiteCore) -> Result<()> {
    ...
    for i in 0..STRESS_MATCH_COUNT {
        let player1 = core.create_test_player(&format!("Stress{}A", i)).await?;
        let player2 = core.create_test_player(&format!("Stress{}B", i)).await?;
...
async fn run_stress_match(core: &TestSuiteCore, player1: TestPlayer, player2: TestPlayer, match_index: usize) -> Result<()> {
    let (challenge, _challenge_event_id) = core.create_and_publish_match_challenge(&player1, 25, (match_index % 4) as u8).await?;
    ...
    core.publish_match_result(&player1, &challenge.match_event_id, Some(winner_npub.clone())).await?;
    core.publish_match_result(&player2, &challenge.match_event_id, Some(winner_npub)).await?;
