...
use crate::matches::{TokenReveal, MoveReveal, MatchChallenge};
...
async fn test_unknown_player_events(core: &TestSuiteCore) -> Result<()> {
    ...
    core.publish_event(&unknown_player, 31002, &fake_reveal).await?;
...
async fn test_duplicate_events(core: &TestSuiteCore) -> Result<()> {
    ...
    let (challenge, _challenge_event_id) = core.create_and_publish_match_challenge(&player1, 100, 0).await?;
    
    // Send acceptance twice to test duplicate handling
    let (_acceptance1, _acceptance_event_id1) = core.create_and_publish_match_acceptance(&player2, &challenge).await?;
    let (_acceptance2, _acceptance_event_id2) = core.create_and_publish_match_acceptance(&player2, &challenge).await?;
...
async fn test_timing_attacks(core: &TestSuiteCore) -> Result<()> {
    ...
    core.publish_event(&player1, 31004, &premature_reveal).await?;
