...
pub async fn test_concurrent_matches(core: &TestSuiteCore) -> Result<()> {
    ...
    for i in 0..3 {
        let player1 = core.create_test_player(&format!("Player{i}A")).await?;
        let player2 = core.create_test_player(&format!("Player{i}B")).await?;
...
async fn process_concurrent_match(
    ...
    core.publish_match_result(&player1, &challenge.match_event_id, Some(winner.clone()))
        .await?;
    core.publish_match_result(&player2, &challenge.match_event_id, Some(winner))
        .await?;
