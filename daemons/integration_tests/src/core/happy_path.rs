...
pub async fn test_happy_path_match(core: &TestSuiteCore) -> Result<()> {
    ...
    core.simulate_combat_rounds(&player1, &player2, &challenge.match_event_id, 3).await?;
    ...
    core.publish_match_result(&player2, &challenge.match_event_id, Some(winner_npub.clone())).await?;
    info!("📋 Phase 6 Complete: Both players submitted agreed match outcome");
    
    // Phase 7: Game Engine Validation & Loot Distribution
    core.verify_loot_distribution(&challenge.match_event_id, &winner_npub).await?;
